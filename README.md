...
### Render-specific note: "Timed Out" / repeated restarts

If your logs show the bot successfully polling Telegram (`getUpdates`
succeeding every ~10 seconds) but Render still says `Timed Out` and restarts
it every few minutes, that's not a bug in the bot — it's because Render's
**Web Service** type expects something to be listening on a port for health
checks, and a pure long-polling Telegram bot doesn't open one.

`bot.py` now starts a tiny built-in HTTP server on Render's expected port
(`$PORT`) purely to satisfy that health check, running alongside the bot's
normal polling loop — no extra setup needed, it's automatic. If your Render
plan supports **Background Worker** services instead of Web Service, that's
an even cleaner fix (no port needed at all) — check your service's Settings
to switch the type if available.
