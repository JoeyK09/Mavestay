...
async def _confirm_booking_payment(context: ContextTypes.DEFAULT_TYPE, booking_id: int) -> str:
    """Marks a booking confirmed, checks referral milestones, notifies the
    referrer if one was just hit. Returns a status line for the admin."""
    booking = db.get_booking(booking_id)
    if not booking:
    ...
    return f"✅ Booking #{booking_id} ({booking['property_name']}) marked as confirmed."
