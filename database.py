    ...
    with get_conn() as conn:
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                referral_confirmed_count INTEGER DEFAULT 0,
                created_at TEXT
            )
        """)
        conn.execute("""
        ...
        conn.executemany(
            """INSERT INTO properties
               (name, city, country, price_per_night, bedrooms, description,
                rating, review_count, tag, is_demo)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)""",
            sample,
        )
...
def add_property(name, city, country, price_per_night, bedrooms, description,
                  owner_name=None, owner_contact=None, owner_payout_method=None,
                  commission_rate=None, rating=4.5, review_count=0, tag=None):
    """Add a real property, optionally with its own commission rate
    (fraction, e.g. 0.15) if this owner negotiated something different
    from your global default. Always marked is_demo=0 (a real listing)."""
    with get_conn() as conn:
        cur = conn.execute(
            """INSERT INTO properties
               (name, city, country, price_per_night, bedrooms, description,
                rating, review_count, tag, owner_name, owner_contact,
                owner_payout_method, commission_rate, is_demo)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)""",
            (name, city, country, price_per_night, bedrooms, description,
             rating, review_count, tag, owner_name, owner_contact,
             owner_payout_method, commission_rate),
        )
...
def add_referral_earning(referrer_id: int, amount: float):
    """Credits a referrer's balance after one of their referred users completes
    a confirmed (paid) booking. Returns (new_balance, new_confirmed_count)."""
    with get_conn() as conn:
        conn.execute(
            """UPDATE users
               SET referral_balance = referral_balance + ?,
                   referral_confirmed_count = referral_confirmed_count + 1
               WHERE telegram_user_id = ?""",
            (amount, referrer_id),
        )
        row = conn.execute(
            """SELECT referral_balance, referral_confirmed_count
               FROM users WHERE telegram_user_id = ?""",
            (referrer_id,),
        ).fetchone()
        return (row["referral_balance"], row["referral_confirmed_count"]) if row else (0, 0)


def get_referral_stats(telegram_user_id: int):
    """Returns (balance, confirmed_count) for a user's own referral earnings."""
    with get_conn() as conn:
        row = conn.execute(
            """SELECT referral_balance, referral_confirmed_count
               FROM users WHERE telegram_user_id = ?""",
...
def compute_commission_split(total_amount, commission_rate):
    """Splits a booking total into (commission_amount, owner_payout_amount)
    based on a fractional rate, e.g. 0.15 for 15%. Returns (None, None) if
    total_amount is unknown (dates couldn't be parsed at inquiry time)."""
    if total_amount is None:
        return None, None
    commission = round(total_amount * commission_rate, 2)
    owner_payout = round(total_amount - commission, 2)
    return commission, owner_payout


def set_booking_commission(booking_id: int, commission_amount, owner_payout_amount):
    with get_conn() as conn:
        conn.execute(
            """UPDATE bookings SET commission_amount = ?, owner_payout_amount = ?
               WHERE id = ?""",
            (commission_amount, owner_payout_amount, booking_id),
        )
...
def get_blocking_bookings(property_id: int):
    """Bookings that occupy dates: payment pending confirmation or confirmed.
    Unpaid inquiries don't block dates — only a payment claim locks them in."""
    with get_conn() as conn:
        return conn.execute(
            """SELECT id, check_in, check_out FROM bookings
               WHERE property_id = ? AND payment_status IN ('pending_confirmation', 'confirmed')""",
            (property_id,),
        ).fetchall()
...
def check_property_availability(property_id, check_in, check_out, exclude_booking_id=None):
    """Returns True if the range is free (or dates couldn't be parsed, in
    which case we can't validate and let it through — admin can catch it
    manually via /pending). Returns False only on a confirmed overlap."""
    d1 = _parse_date(check_in)
    d2 = _parse_date(check_out)
    if not d1 or not d2 or d1 >= d2:
        return True

    for row in get_blocking_bookings(property_id):
        if exclude_booking_id and row["id"] == exclude_booking_id:
            continue
        e1 = _parse_date(row["check_in"])
        e2 = _parse_date(row["check_out"])
        if not e1 or not e2:
            continue
        if d1 < e2 and e1 < d2:  # overlap
            return False
    return True
